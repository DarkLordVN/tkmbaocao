        ...
        public List<ThongBaoViewModel> GetList(Expression<Func<ThongBao, bool>> where, int pageIndex, int pageSize, ref int totalItem)
        {
            try
            {
                ...
                if (leReturn != null && leReturn.Count > 0)
                {
                    ...
                    {
                        if (item.NguoiTaoID == item.NguoiDung.ID)
                        {
                            item.TenNguoiGui = item.NguoiDung.HoVaTen;
                            //item.ChucVuNguoiGui = item.NguoiDung.DmChucVu.TenChucVu;
                            item.Avatar = item.NguoiDung.AnhDaiDien;
                        }
                    }
