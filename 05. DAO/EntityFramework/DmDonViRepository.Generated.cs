...
{   
	public partial class DmDonViRepository
	{
		private IRepository<DmDonVi> _repository {get;set;}
		public IRepository<DmDonVi> Repository
		{
			get { return _repository; }
			set { _repository = value; }
		}
		
		public DmDonViRepository(IRepository<DmDonVi> repository, IUnitOfWork unitOfWork)
    	{
    		Repository = repository;
			Repository.UnitOfWork = new EFUnitOfWork(); 
    	}

		public List<DmDonVi> GetList(Expression<Func<DmDonVi, bool>> where)
        {
            try
            {
                IEnumerable<DmDonVi> lResult;
                if (where != null)
                {
		...
		public List<DmDonVi> GetList(Expression<Func<DmDonVi, bool>> where, int pageIndex, int pageSize, ref int totalItem, Func<DmDonVi, object> orderCol = null, bool isDecending = true)
        {
            try
            {
                IEnumerable<DmDonVi> lResult;
        ...
        public DmDonVi GetByFilter(Expression<Func<DmDonVi, bool>> where)
        {
            try
            {
                Repository.Save();
		...
		public bool CheckByFilter(Expression<Func<DmDonVi, bool>> where)
        {
            try
            {
                Repository.Save();
        ...
        public bool Add(DmDonVi entity)
        {
		...
		public bool Add(List<DmDonVi> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
        ...
        public bool Update(DmDonVi entity)
        {
            try
            {
		...
		public bool Update(List<DmDonVi> lentity)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
