...
namespace TKM.WebApp.Controllers
{
    public class AuthenticationController : Controller
    {
        ...
        private readonly DmDonViService _dmDonViService;
                ...
                if (!string.IsNullOrEmpty(m.RedirectUrl) || (string.IsNullOrEmpty(m.RedirectUrl) && (!string.IsNullOrEmpty(m.TenDangNhap) || !string.IsNullOrEmpty(m.MatKhau))))
                {
                    ...
                    else
                    {
                        ...
                        if (isOk)
                        {
                            string typeUser = "";
                            string capBac = "";
                            var objDonVi = _dmDonViService.GetById(acc.DonViID);
                            if (acc.IsTongHop) typeUser = "th";
                            if (objDonVi != null)
                            {
                                // thuộc Cục : KhoaChaID = 0
                                if (objDonVi.KhoaChaID == 0)
                                {
                                    if (acc.IsLanhDao) typeUser = "ldc";
                                    else if (acc.IsVanThu) typeUser = "vtc";
                                }
                                //Thuộc đơn vị
                                else
                                {
                                    if (acc.IsLanhDao)
                                        typeUser = "lddv";
                                    else if (acc.IsVanThu)
                                        typeUser = "vtdv";
                                }
                                //Văn thư cục
                                if (acc.IsVanThu) if (!objDonVi.IsDonVi) typeUser = "vtc";
                            }
                            if (acc.IsTruong) capBac = "truong";
                            else if (acc.IsPho) capBac = "pho";
                            else if (typeUser == "") capBac = "cb";
                            if (acc != null && acc.DonViID > 0)
                            {
                                var dmDonVi = new DmDonViService();
                                var vmDonVi = dmDonVi.GetById(acc.DonViID);
                                if (vmDonVi != null)
                                {
                                    acc.TenDonVi = vmDonVi.TenDonVi;
                                    acc.IsCuc = !vmDonVi.IsDonVi;
                                }
                            }
        ...
        [HttpPost]
        public ActionResult Loginv1(LoginModel m, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                var strLocked = "";
